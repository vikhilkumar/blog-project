...
urlpatterns = [
    ...
    url(r'^details/(?P<id>\d+)/$', views.post_details),
    url(r'^mailsendview/(?P<id>\d+)/$', views.mail_send_view),

    url(r'^(?P<year>\d{4})/(?P<month>\d{2})/(?P<day>\d{2})/(?P<post>[-\w]+)/$', views.post_detail_view,name='post_detail'),
           #To pass the values to above objects, we have created get_absolute_url in models.py from function will will pass the values.
